...
%% Iterative algorithm for phase retrieval PR                   
    [tau,rms_angle,rms_fourier] = PR(Ei,Et,thresh,b,t,NLx,NLy,steps);
...
% Discretization of the mask T
    ...
    Er_d = FFT2(Ei.*exp(1i*(fase_T_dis*2*pi-pi))); % field for the discretized mask
    Er = FFT2(Ei.*tau); % recovered field
...
%% Quality index determination
        ...
        Intensity_recovered = round(mat2gray(abs(Er_d(Ny/2-NLy/2+OffsetY:Ny/2+NLy/2-1+OffsetY,Nx/2-NLx/2+OffsetX:Nx/2+NLx/2-1+OffsetX)))*(2^alfa-1));
        Phase_recovered = round(mat2gray(angle(Er_d(Ny/2-NLy/2+OffsetY:Ny/2+NLy/2-1+OffsetY,Nx/2-NLx/2+OffsetX:Nx/2+NLx/2-1+OffsetX)))*(2^alfa-1));
